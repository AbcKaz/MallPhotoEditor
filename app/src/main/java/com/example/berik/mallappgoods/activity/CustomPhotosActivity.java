...
public class CustomPhotosActivity extends Activity {
    ...
    private void init() {
        ...
        }else  if (action.equalsIgnoreCase(Helper.ACTION_PICK)) {
    ...
    AdapterView.OnItemClickListener mItemMulClickListener = new AdapterView.OnItemClickListener() {
        ...
        public void onItemClick(AdapterView<?> l, View v, int position, long id) {
            ...
            Log.d("Imgselected:",String.valueOf(adapter.getSelected().size()));

            if(adapter.getSelected().size()<=3) {
                adapter.changeSelection(v, position);
            }else{
                Toast.makeText(getApplicationContext(), "Вы можете выбрать максимум 4 изображений!!! len=" + adapter.getSelected().size(),Toast.LENGTH_SHORT).show();
            }
        }
    };
    ...
    private ArrayList<CustomPhotos> getGalleryPhotos() {
        ArrayList<CustomPhotos> galleryList = new ArrayList<>();
